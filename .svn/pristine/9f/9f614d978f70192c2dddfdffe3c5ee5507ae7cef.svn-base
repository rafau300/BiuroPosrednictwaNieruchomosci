 ...
 */
public class Serwer implements Serializable, Runnable {
	...
	/**
	 * Ta funkcja w trakcie oczekiwania na klienta wyswietla kropki na ekranie,
	 * zeby uzytkownik serwera wiedzial, ze serwer dziaa prawidlowo i moze
	 * przyjmowac nowe polaczenia
	 */
	...
	public void wyswietlSzczegoloweDaneKlienta(int id, String[] dane,
			String[] kolumny) {
		baza.wyswietlSzczegoloweDaneKlienta(id, dane, kolumny);
	}

	public void wyswietlNieruchomosci(String skrypt, String[] miejscowosc,
			String[] ulica, int[] cena) {
		baza.wyswietlNieruchomosci(skrypt, miejscowosc, ulica, cena);
	}

	public void wyswietlSzczegoloweDaneNieruchomosci(int id, String[] dane,
			String[] kolumny) {
	 ...
	 * @param args
	 * @throws Funkcja
	 *             wyrzuca wyjatki w przypadku problemow z siecia
	 */
	...
	public void run() {
		...
		while (true) {
			...
			i++;
			flaga = false;
			new Wymiana(klient, i).start(); // start nowego watku dla kazdego z
											// klientow
		}
	}
...
/**
 * Klasa pozwalajaca na przesylanie danych w oddzielnych watkach dla kazdego z
 * klientow
 * 
 * @author Rafal
 * 
		...
		Thread polaczenie = new Thread(new Runnable() {// tworzenie nowego watku
					...
					public void run() {
						...
						try {
							...
							System.out
									.println(">>Wysylam potwierdzenie do klienta nr: "
											+ i);
							klient.getOutputStream().write(1111);// wysy�anie
																	// do graczy
							// server.close();
						} catch (Exception e) {
							e.printStackTrace();
						...
						try {
							out = new ObjectOutputStream(klient
									.getOutputStream());
						...
						try {
							...
							System.err.println("Blad logowania klienta nr: "
									+ i);
						}

						if (logowanie[i].login.equals("pracownik")
								&& logowanie[i].haslo.equals("haslo")) {
							logowanie[i].czyZalogowano = true;
							System.out
									.println("Klient " + i
											+ " poprawnie sie zalogowal"
											+ " jako uzytkownik: "
											+ logowanie[i].login);
						} else {
							if (logowanie[i].wiadomoscZwrotna == null)
								logowanie[i].wiadomoscZwrotna = "";

							if (!logowanie[i].login
									.equalsIgnoreCase("pracownik")) {
						...
						try {
							...
							// TODO Auto-generated catch block
							System.err
									.println("Blad wysylania informacji o logowaniu klienta nr: "
											+ i);
						}

						/*
						 * try { serverSerializacji.close(); //server.close(); }
						...
						do {
							...
							try {
								out = new ObjectOutputStream(klient
										.getOutputStream());
								in = new ObjectInputStream(klient
										.getInputStream());
							} catch (IOException e1) {
								// TODO Auto-generated catch block
								e1.printStackTrace();
							...
							try {
								...
								System.err
										.println("Blad przy odbiorze pakietu od klienta nr: " + i);
								wiadomosc[i].koniec = true;
							}
							...
							if (wiadomosc[i].koniec == true) {
								...
								if (liczbaPodlaczonychKlientow == 0) {
									System.out
											.println("Wszyscy klienci sie wylogowali\nOczekiwanie na kolejnych...");
									serwer.flaga = true;
							...
							if (wiadomosc[i].idCzynnosciDoWykonania == 1) {
								serwer.wyswietlKlientow(wiadomosc[i].skryptSQL,
										wiadomosc[i].imiona,
										wiadomosc[i].nazwiska);
								wiadomosc[i].idCzynnosciDoWykonania = 0;
								wiadomosc[i].skryptSQL = null;
							}

							if (wiadomosc[i].idCzynnosciDoWykonania == 2) {
								serwer.wyswietlSzczegoloweDaneKlienta(
										wiadomosc[i].idKlienta,
										wiadomosc[i].dane, wiadomosc[i].kolumny);
								wiadomosc[i].idCzynnosciDoWykonania = 0;
							}
							
							if (wiadomosc[i].idCzynnosciDoWykonania == 3) {
								serwer.wyswietlNieruchomosci(wiadomosc[i].skryptSQL, 
										wiadomosc[i].miejscowosc, wiadomosc[i].ulica, 
										wiadomosc[i].cena);
								wiadomosc[i].idCzynnosciDoWykonania = 0;
								wiadomosc[i].skryptSQL = null;
							}
							
							if (wiadomosc[i].idCzynnosciDoWykonania == 4) {
								serwer.wyswietlSzczegoloweDaneNieruchomosci(wiadomosc[i].idNieruchomosci, 
										wiadomosc[i].dane, wiadomosc[i].kolumny);
								wiadomosc[i].idCzynnosciDoWykonania = 0;
								wiadomosc[i].skryptSQL = null;
							}
							
							if (wiadomosc[i].idCzynnosciDoWykonania == 5) {
								serwer.insert(wiadomosc[i].skryptSQL);
								wiadomosc[i].idCzynnosciDoWykonania = 0;
								wiadomosc[i].skryptSQL = null;
							}
							
							if (wiadomosc[i].idCzynnosciDoWykonania == 6) {
								serwer.zakupNieruchomosci(wiadomosc[i].idKlienta, 
										wiadomosc[i].idNieruchomosci);
								wiadomosc[i].idCzynnosciDoWykonania = 0;
								wiadomosc[i].skryptSQL = null;
							}
							...
							try {
								...
								// TODO Auto-generated catch block
								System.err
										.println("Blad przy wysylaniu pakietu do klienta nr: "
												+ i);
							}
